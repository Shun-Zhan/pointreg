...
def _effective_trim_fraction(distances: np.ndarray, config: RegistrationConfig) -> float:
    """Tighten the trim fraction when the inlier ratio suggests low overlap.

    With little true overlap, correspondences near the correspondence-distance
    boundary are mostly wrong and drag the solution away; keeping only the best
    fraction proportional to the estimated overlap is more robust.
    """
    if not getattr(config, "adaptive_trim", False) or not len(distances):
